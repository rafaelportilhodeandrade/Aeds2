...
// comparar e swap
int compararTexto(Texto a, Texto b) {
    int i = 0;
    while (a.conteudo[i] != '\0' && b.conteudo[i] != '\0') {
        if (a.conteudo[i] < b.conteudo[i]) return -1;
        if (a.conteudo[i] > b.conteudo[i]) return 1;
        i++;
    }
    if (a.conteudo[i] == '\0' && b.conteudo[i] == '\0') return 0;
    if (a.conteudo[i] == '\0') return -1;
    return 1;
}
...
// Selection Sort com contagem de comparações e movimentações
void selecao(Jogo jogos[], int cont, long *comparacoes, long *movimentacoes) {
    *comparacoes = 0;
    *movimentacoes = 0;
    ...
    for (int i = 0; i < cont - 1; i++) {
        ...
        for (int j = i + 1; j < cont; j++) {
            (*comparacoes)++;
            if (compararTexto(jogos[j].titulo, jogos[menor].titulo) < 0) {
                menor = j;
            }
        }
        if (menor != i) {
            swap(&jogos[i], &jogos[menor]);
            (*movimentacoes)++;
        }
    }
}
...
int main() {
    ...
    Jogo *listaTemporaria = malloc(100 * sizeof(Jogo));
    ...
    while(strcmp(busca.conteudo,"FIM")!=0){
        ...
        for(int i=0;i<total;i++){
            if(listaJogos[i].id==id){
                listaTemporaria[cont] = listaJogos[i];
                cont++;
                i = total;
            }
        }
    ...
    selecao(listaTemporaria, cont, &comparacoes, &movimentacoes);
    ...
    // imprime jogos
    for(int i=0;i<cont;i++) mostrarJogo(&listaTemporaria[i]);

    // escreve arquivo
    FILE *out = fopen("suamatricula_selecao.txt","w");
    if(out){
        fprintf(out,"890258\n");
        fprintf(out,"Tempo de execução: %.0f ms\n", tempo);
        fprintf(out,"Número de comparações: %ld\n", comparacoes);
        fprintf(out,"Número de movimentações: %ld\n", movimentacoes);
    }
    fclose(out);
