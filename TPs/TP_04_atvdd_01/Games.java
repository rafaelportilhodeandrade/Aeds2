...
public class Games {
    ...
    public static String formatarData(String data) {
        ...
        String mesTexto = (partes.length > 0 && !partes[0].trim().isEmpty()) ? partes[0].trim() : "Jan";

        String dia = (partes.length > 1 && !partes[1].trim().isEmpty())
                ? partes[1].replace(",", "").trim()
                : "01";

        String ano = (partes.length > 2 && !partes[2].trim().isEmpty())
                ? partes[2].trim()
                : "0001";
    ...
    public static Games formatar(String linha) {
        ...
        // players
        temp = separar(linha, pos);
        int players = temp[0].trim().isEmpty() ? 0 : Integer.parseInt(temp[0].trim());
        pos = Integer.parseInt(temp[1]);

        // preço
        temp = separar(linha, pos);
        float preco = temp[0].trim().isEmpty() ? 0.0f : Float.parseFloat(temp[0].trim());
        pos = Integer.parseInt(temp[1]);
        ...
        // nota crítica
        temp = separar(linha, pos);
        int notaCritica = temp[0].trim().isEmpty() ? -1 : Integer.parseInt(temp[0].trim());
        pos = Integer.parseInt(temp[1]);

        // nota usuários
        temp = separar(linha, pos);
        float notaUsuarios = temp[0].trim().isEmpty() ? -1.0f : Float.parseFloat(temp[0].trim());
        pos = Integer.parseInt(temp[1]);

        // conquistas
        temp = separar(linha, pos);
        int conquistas = temp[0].trim().isEmpty() ? 0 : Integer.parseInt(temp[0].trim());
        pos = Integer.parseInt(temp[1]);
